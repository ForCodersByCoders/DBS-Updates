...
const isBoosting = async (client, message, args, name, code, channel) => {
    ...
    if (code.split("$isBoosting")[r].startsWith("[")) {
        ...
        if (!member && (inside.split(";").length === 1)) return message.channel.send(`:x: Invalid user ID in \`$isBoosting[${inside}]\`.\n${docs.conditions}/isboosting`)
        if (!member && (inside.split(";").length === 2)) return message.channel.send(`:x: Invalid user ID in 1st field of \`$isBoosting[${inside}]\`.\n${docs.conditions}/isboosting`)
