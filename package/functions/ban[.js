...
const ban = async (client, message, args, name, code) => {
    ...
    if (!guild) return message.channel.send(`:x: Invalid guild ID in 2nd field of \`$ban[${inside}]\`.\n${docs.action}/ban`)

    let id = (userID ? userID : message.author.id)
    let member = await client.users.fetch(id).catch(err => { })


    if (!member && err === undefined) return message.channel.send(`:x: Invalid user ID in 1st field of \`$ban[${inside}]\`.\n${docs.action}/ban`)
