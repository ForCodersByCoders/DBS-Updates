...
const guildChannels = async (client, message, args, name, code) => {
    ...
    if (code.split("$guildChannels")[r].startsWith("[")) {
        ...
        if (option === "mention") {
            code = code.replaceLast(`$guildChannels[${inside}]`, guild.channels.cache.filter(channel => (channel.id !== message.guild.id) && (channel.type !== "category")).map(channel => `<#${channel.id}>`).join(", "))
        } else if (option === "id") {
            code = code.replaceLast(`$guildChannels[${inside}]`, guild.channels.cache.filter(channel => (channel.id !== message.guild.id) && (channel.type !== "category")).map(channel => channel.id).join(", "))
        } else {
            code = code.replaceLast(`$guildChannels[${inside}]`, guild.channels.cache.filter(channel => (channel.id !== message.guild.id) && (channel.type !== "category")).map(channel => channel.name).join(", "))
        }
