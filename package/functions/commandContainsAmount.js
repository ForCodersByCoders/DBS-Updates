const commandContainsAmount = async (client, message, args, name, code) => {

    const r = code.split("$commandContainsAmount").length - 1

        let inside = code.split("$commandContainsAmount[")[r].split(/]$/gm)[0]
        ...
        final = result.split(contains).length - 2

        code = code.replaceLast(`$commandContainsAmount[${inside}]`, final)

        return {
            code: code,
        }
}
module.exports = commandContainsAmount;