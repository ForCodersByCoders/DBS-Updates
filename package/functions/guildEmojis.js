...
const guildEmojis = async (client, message, args, name, code) => {
    ...
    if (code.split("$guildEmojis")[r].startsWith("[")) {
        ...
        if (option === "name") {
            code = code.replaceLast(`$guildEmojis[${inside}]`, guild.emojis.cache.filter(emoji => emoji.id !== message.guild.id).map(emoji => emoji.name).join(", "))
        } else if (option === "id") {
            code = code.replaceLast(`$guildEmojis[${inside}]`, guild.emojis.cache.filter(emoji => emoji.id !== message.guild.id).map(emoji => emoji.id).join(", "))
        } else {
            code = code.replaceLast(`$guildEmojis[${inside}]`, guild.emojis.cache.filter(emoji => emoji.id !== message.guild.id).map(emoji => emoji.toString()).join(", "))
        }
