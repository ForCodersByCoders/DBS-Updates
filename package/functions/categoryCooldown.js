...
const categoryCooldown = async (client, message, args, name, code) => {

    if (code.split("$categoryCooldown[").length >= 3) return message.channel.send(`❌ Can't use more than one $categoryCooldown.\n${docs.limiters}/categorycooldown`)

    let inside = code.split("$categoryCooldown[")[1].split(/]$/gm)[0]
    ...
    let item = await db.fetch(`${name}_${message.channel.parent.id}`)
    ...
    } else db.set(`${name}_${message.channel.parent.id}`, Date.now())

    code = code.replace(`$categoryCooldown[${inside}]`, "")
...
module.exports = categoryCooldown;