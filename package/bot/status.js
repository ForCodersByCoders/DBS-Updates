...
const Status = async (client, ops, time) => {
    ...
    if (isNaN(time) || time < 10000) return console.error(chalk.redBright('API limitation reached:'), `Status change cannot be faster (less) than 10 seconds / 10000`)
    ...
    setInterval(async () => {
        ...
        else if (arr[y][1].type === "COMPETING") {
            client.user.setPresence({
                activity: {
                    name: code,
                    type: 5
